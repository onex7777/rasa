...
class TicketFormAction(FormAction):
    def name(self) -> Text:
        return "train_query_form"

    def required_slots(self, tracker: Tracker) -> List[Text]:
        return ["departure", "destination", "date-time"]
    ...
    def slot_mappings(self):
        return {
            "departure": [
                self.from_entity(entity="address", role="departure"),
                self.from_entity(entity="address", intent="info_address"),
            ],
            "destination": [
                self.from_entity(entity="address", role="destination"),
                self.from_entity(entity="address", intent="info_address"),
            ],
        }
...
class ActionQueryTrain(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        #     dispatcher.utter_message(text="抱歉，服务器不通，我无法从服务器获取订票信息！")
        #     return []
        #
        # data = response.json()
        #

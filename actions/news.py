...
class ActionNews(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        if response.status_code != 200:
            dispatcher.utter_message(text="抱歉，服务器不通，我无法从服务器获取新闻数据！")
            return []
...
class ActionWeiboHot(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        if response.status_code != 200:
            dispatcher.utter_message(text="抱歉，服务器不通，我无法从服务器获取微博热点数据！")
            return []
...
class ActionToutiaoHot(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        if response.status_code != 200:
            dispatcher.utter_message(text="抱歉，服务器不通，我无法从服务器获取头条热点数据！")
            return []
